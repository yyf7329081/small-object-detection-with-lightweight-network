...
from external.gpu_nms.nms.gpu_nms import gpu_nms
from external.cpu_nms.cpu_nms import cpu_soft_nms
from model.rpn.bbox_transform import bbox_transform_inv
from model.utils.net_utils import save_net, load_net, vis_detections
from model.utils.blob import im_list_to_blob

from model.faster_rcnn.Snet_dla import snet
...
if __name__ == '__main__':
    ...
    set_cfgs = [
            'ANCHOR_SCALES', '[0.5, 1 , 2, 4, 8]', 'ANCHOR_RATIOS', '[1.0/2 , 3.0/4 , 1 , 4.0/3 , 2 ]',
    ...
    input_dir = os.path.join(cfg.ROOT_DIR, 'weights', cfg.EXP_DIR, args.net, args.dataset)
    ...
    visdrone_classes = np.asarray(['__background__', 'pedestrian', 'people', 'bicycle', 'car',
                'van', 'truck', 'tricycle', 'awning-tricycle', 'bus', 'motor'])   
    nwpu_classes = np.asarray(['__background__', 'airplane', 'ship', 'storage tank', 'baseball diamond',
                'tennis court', 'basketball court', 'ground track field', 'harbor', 'bridge', 'vehicle']) 
    vedai_classes = np.asarray(['__background__', 'car', 'truck', 'pickup', 'tractor',
                'camping car', 'boot', 'motorcycle', 'bus', 'van', 'other', 'plane'])
    car_3class_classes = np.asarray(['__background__', 'car', 'bus', 'truck']) 
    
    if args.dataset == 'visdrone':
        classes = visdrone_classes
    if args.dataset == 'vedai':
        classes = vedai_classes
    if args.dataset == 'nwpu':
        classes = nwpu_classes        
    if args.dataset == 'pascal_voc':
        classes = pascal_classes
    if args.dataset == 'car_3class':
        classes = car_3class_classes
    
    layer = int(args.net.split("_")[1])
    _RCNN = snet(classes,layer, pretrained_path= None  , class_agnostic=args.class_agnostic)
    ...
    while (num_images >= 0):
        ...
        for j in xrange(1, len(classes)):
            ...
            if inds.numel() > 0:
                ...
                _, order = torch.sort(cls_scores, 0, True)# 降序
                if args.class_agnostic:
                ...
                keep = cpu_soft_nms(cls_dets_np, sigma=0.3, Nt=0.3, threshold=0.1, method=1)
                cls_dets_np = cls_dets_np[keep]
                #keep = torch.from_numpy(np.array(keep))
                #cls_dets = cls_dets[keep.view(-1).long()]
                if vis:
                    #vis_detections(im2show, classes[j],color_list[j].tolist(),
        ...
        if webcam_num == -1:
            sys.stdout.write('im_detect: {:03d}/{:03d} Detect: {:.3f}s (RPN: {:.3f}s, Pre-RoI: {:.3f}s, RoI: {:.3f}s, Subnet: {:.3f}s), NMS: {:.3f}s\n' \
        ...
        if vis and webcam_num == -1:
            ...
            result_path = os.path.join(args.output_dir,
                                       os.path.splitext(imglist[num_images])[0] + "_det.jpg")
            #result_path = result_path.replace("input","output")
            #cv2.imwrite(result_path, im2show)
            imsave(result_path, im2show)
        else:
