...
from .visdrone_eval import visdrone_eval
...
class visdrone(imdb):
    def __init__(self, image_set, year, devkit_path=None):
        ...
        if image_set == 'trainset':
            self.dataset_dir = 'VisDrone2018-DET-train'
        elif image_set == 'valset':
            self.dataset_dir = 'VisDrone2018-DET-val'
        elif image_set == 'testset':
            self.dataset_dir = 'VisDrone2018-DET-test-dev'
        else:
            print('Error: have no image_set')
        self._data_path = os.path.join(self._devkit_path, 'VisDrone' + self._year, self.dataset_dir)
        self._classes = (
    ...
    def _do_python_eval(self, output_dir='output'):
        ...
        for i, cls in enumerate(self._classes):
            if cls == '__background__':
                continue
            filename = self._get_voc_results_file_template(output_dir).format(cls)
            rec, prec, ap = visdrone_eval(filename,# 每一类的测试结果，图片名，得分，四个坐标
                                     annopath,
                                     imagesetfile,
                                     i,
                                     cls,
