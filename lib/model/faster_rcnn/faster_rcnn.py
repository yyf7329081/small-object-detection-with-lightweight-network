...
def draw_features(channels,x,save_name):
    for i in range(channels):
        ...
        result_path = os.path.join(cfg.ROOT_DIR, 'pictures', 'vis_feat', save_name + '%d.jpg'%i)
        cv2.imwrite(result_path, img_resize)
...
class _fasterRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes,
        ...
        start = time.time()
        basefeat = self.RCNN_base(im_data)# snet
        

        #draw_features(5,basefeat.cpu().numpy(),'before')        

        # feed base feature map tp RPN to obtain rois
        rpn_feat= self.rpn(basefeat)# 5x5分离卷积


        # rois, rpn_loss_cls, rpn_loss_bbox = self.RCNN_rpn(rpn_feat, im_info, gt_boxes, num_boxes,hm,reg_mask,wh,offset,ind)
        # rois:(bs,post_nms_topN,5)
        ...
        #draw_features(5,basefeat.cpu().numpy(),'after')
