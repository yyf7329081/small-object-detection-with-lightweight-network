...
class _AnchorTargetLayer(nn.Module):
    ...
    def __init__(self, feat_stride, scales, ratios):
        ...
        self.rpn_negative_overlap = cfg.TRAIN.RPN_NEGATIVE_OVERLAP # 与gt的iou<0.3，标定为负样本
        self.rpn_positive_overlap = cfg.TRAIN.RPN_POSITIVE_OVERLAP # 与gt的iou>0.7，标定为正样本
        self.rpn_fg_fraction = cfg.TRAIN.RPN_FG_FRACTION # 正样本数量的比例
    ...
    def forward(self, input):
        ...
        inds_inside = torch.nonzero(keep).view(-1)# 保存的不越界的anchor的索引(num_keeep_anchor,)

        # keep only inside anchors
        anchors = all_anchors[inds_inside, :]

        # label: 1 is positive, 0 is negative, -1 is dont care
        labels = gt_boxes.new(batch_size, inds_inside.size(0)).fill_(-1)# (bs,num_keeep_anchor)
        bbox_inside_weights = gt_boxes.new(batch_size, inds_inside.size(0)).zero_()# (bs,num_keeep_anchor)
        bbox_outside_weights = gt_boxes.new(batch_size, inds_inside.size(0)).zero_()# (bs,num_keeep_anchor)

        overlaps = bbox_overlaps_batch(anchors, gt_boxes)# 计算anchors和gt_boxes的iou，(bs,num_keep_anchor,max_num_gt)

        max_overlaps, argmax_overlaps = torch.max(overlaps, 2)# 对每个anchor求iou最大的gt，(bs, num_keep_anchor)
        gt_max_overlaps, _ = torch.max(overlaps, 1)# 对每个gt，求iou最大的anchor, (bs, num_gt)

        if not cfg.TRAIN.RPN_CLOBBER_POSITIVES:# 如果anchor和所有gt的iou最大值< cfg.TRAIN.RPN_NEGATIVE_OVERLAP，把该anchor标定为负样本
            labels[max_overlaps < cfg.TRAIN.RPN_NEGATIVE_OVERLAP] = 0

        gt_max_overlaps[gt_max_overlaps==0] = 1e-5
        keep = torch.sum(overlaps.eq(gt_max_overlaps.view(batch_size,1,-1).expand_as(overlaps)), 2)# (bs,num_keep_anchor)，保留每个gt对应iou最大的anchor为1，其他为0

        if torch.sum(keep) > 0:
            labels[keep>0] = 1

        # fg label: above threshold IOU，与gt的最大iou>0.7的，标定为正样本
        ...
        sum_fg = torch.sum((labels == 1).int(), 1)# 正样本数量
        sum_bg = torch.sum((labels == 0).int(), 1)# 负样本数量

        for i in range(batch_size):
            # subsample positive labels if we have too many，
            if sum_fg[i] > num_fg:# 如果正样本过多，打乱随机选择128个，剩下的标-1
...
#           num_bg = cfg.TRAIN.RPN_BATCHSIZE - sum_fg[i]
        ...
        bbox_inside_weights[labels==1] = cfg.TRAIN.RPN_BBOX_INSIDE_WEIGHTS[0] # 正样本权重为1，其他为0

        if cfg.TRAIN.RPN_POSITIVE_WEIGHT < 0:
            num_examples = torch.sum(labels[i] >= 0)# 总样本个数
            positive_weights = 1.0 / num_examples.item()# 对于分类损失，正样本的权重
            negative_weights = 1.0 / num_examples.item()# 分类损失，负样本的权重
        else:
        ...
        labels = _unmap(labels, total_anchors, inds_inside, batch_size, fill=-1)#(bs,num_anchors),映射回所有anchor
        bbox_targets = _unmap(bbox_targets, total_anchors, inds_inside, batch_size, fill=0)#(bs,num_anchors,4)，保存边界内anchor的目标偏移参数
        bbox_inside_weights = _unmap(bbox_inside_weights, total_anchors, inds_inside, batch_size, fill=0)#(bs,num_anchors)
        bbox_outside_weights = _unmap(bbox_outside_weights, total_anchors, inds_inside, batch_size, fill=0)#(bs,num_anchors)

        outputs = []

        labels = labels.view(batch_size, height, width, A).permute(0,3,1,2).contiguous()
        labels = labels.view(batch_size, 1, A * height, width)
        outputs.append(labels)#(bs, 1, A*height, width),所有anchor的label。正样本1，负样本0，忽略-1

        bbox_targets = bbox_targets.view(batch_size, height, width, A*4).permute(0,3,1,2).contiguous()
        outputs.append(bbox_targets)#(bs, A*4, height, width)，所有anchor的真实偏移参数。

        anchors_count = bbox_inside_weights.size(1)
        bbox_inside_weights = bbox_inside_weights.view(batch_size,anchors_count,1).expand(batch_size, anchors_count, 4)

        bbox_inside_weights = bbox_inside_weights.contiguous().view(batch_size, height, width, 4*A)\
                            .permute(0,3,1,2).contiguous()# (bs, A*4, height, width)
