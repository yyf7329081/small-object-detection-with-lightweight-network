...
# __C.TRAIN.CLASS_WEIGHT = [0.85292781, 2.30858181, 2.33600029, 0.12259726, 0.49622705, 0.38326303, 1.25462266, 1.20836015, 0.39743137, 1.39481366]
#
# Testing options
#
__C.TEST = edict()
...
def get_output_wt_dir(dataset_name, net_name):
  ...
  """
    if net_name is None:
        net_name = 'default'
    outdir = osp.join(__C.ROOT_DIR, 'weights', __C.EXP_DIR, net_name, dataset_name)  
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    return outdir

def get_output_log_dir(dataset_name, net_name):
    """Return the directory where experimental artifacts are placed.
  If the directory does not exist, it is created.
  ...
  """
    if net_name is None:
        net_name = 'default'
    outdir = osp.join(__C.ROOT_DIR, 'logs', __C.EXP_DIR, net_name, dataset_name)  
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    return outdir

def get_output_dir(dataset_name, net_name):
    """Return the directory where experimental artifacts are placed.
  If the directory does not exist, it is created.
  ...
  """
    if net_name is None:
        net_name = 'default'
    outdir = osp.join(__C.ROOT_DIR, 'eval_cache', __C.EXP_DIR, net_name, dataset_name)
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    return outdir
