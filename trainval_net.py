...
from model.utils.config import cfg, cfg_from_file, cfg_from_list, get_output_dir,  \
    get_output_wt_dir, get_output_log_dir, _merge_a_into_b
...
if __name__ == '__main__':
    ...
    if args.dataset == "visdrone":
        args.imdb_name = "visdrone_2018_train+visdrone_2018_val"
        args.imdbval_name = "visdrone_2018_test"
        # args.set_cfgs = [
    ...
    if args.dataset == "nwpu":
        args.imdb_name = "nwpu_2018_train+nwpu_2018_val"
        args.imdbval_name = "nwpu_2018_test"
        # args.set_cfgs = [
        #     'ANCHOR_SCALES', '[8, 16, 32]', 'ANCHOR_RATIOS', '[0.5,1,2]',
        #     'MAX_NUM_GT_BOXES', '20'
        # ]
    ...
    if args.dataset == "vedai":
        args.imdb_name = "vedai_2018_train+vedai_2018_val"
        args.imdbval_name = "vedai_2018_test"
        # args.set_cfgs = [
        #     'ANCHOR_SCALES', '[8, 16, 32]', 'ANCHOR_RATIOS', '[0.5,1,2]',
        #     'MAX_NUM_GT_BOXES', '20'
        # ]
    ...
    if args.dataset == 'car_3class':
        args.imdb_name = "car_3class_2019_trainset+car_3class_2019_valset"
        args.imdbval_name = "car_3class_2019_testset"
        # args.set_cfgs = [
        #     'ANCHOR_SCALES', '[8, 16, 32]', 'ANCHOR_RATIOS', '[0.5,1,2]',
        #     'MAX_NUM_GT_BOXES', '20'
        # ]
        args.set_cfgs = [
            'ANCHOR_SCALES', '[1, 2 , 4, 8, 16]', 'ANCHOR_RATIOS', '[1.0/2 , 3.0/4 , 1 , 4.0/3 , 2 ]',
            'MAX_NUM_GT_BOXES', '100'
        ]            
    ...
    output_dir = get_output_wt_dir(args.dataset, args.net)
    log_dir = get_output_log_dir(args.dataset, args.net)
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            if step % (min(100, int(iters_per_epoch/4))) == 0 and step > 0 and epoch == 0 and warm:
            ...
            if step % args.disp_interval == 0:
                ...
                print("[%d:%d][epoch %2d][iter %4d/%4d] loss: %.4f, lr: %.2e, rpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f size:(%3d,%3d)" \
                    % (now_time.tm_hour, now_time.tm_min, epoch, step, iters_per_epoch, loss_temp, lr, loss_rpn_cls, loss_rpn_box, loss_rcnn_cls, loss_rcnn_box,sampler_batch.size,sampler_batch.size))
                ...
                loss_temp = 0
                start = time.time()
        if (epoch+1)%args.eval_interval==0:# 每eval_interval个epoch保存一个权重文件    
            if args.mGPUs:
